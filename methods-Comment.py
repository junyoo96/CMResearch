...
#weight의 초기값을 설정해주는 코드 
def weights_init(m):
    classname = m.__class__.__name__
    if classname.find('Linear') != -1:
        #fc layer면 평균이 0이고 표준편차가 0.02인 정규분포에서 값을 뽑아냄 
...
class CuMix:
    ...
    def __init__(self, seen_classes, unseen_classes, attributes, configs, zsl_only=False, dg_only=False,
        ...
        if configs['backbone'] == 'none':
            ...
            #eval : backbone이 무엇인지 출력하기 
            backbone = eval(configs['backbone'])
            #resnet pretrained된 모델 사용
            #backbone = ResNet18 모델 
        ...
        if dg_only:
            ...
            #classifier는 fully-connected layer
    ...
    def forward(self, input, return_features=False):
        #f 지나간거
        features = self.backbone(input)
        #semantic_projector : g
        #train_classifier : 오메가
    ...
    def get_sample_mixup(self, domains):
        ...
        #얼마나 많은 도메인들이 각 batch에 있는지 확인 
        if self.dpb>1:
            #batch에 있는 domains 수만큼
                #domains=['photo', 'art_painting', 'cartoon'] 현재 이 상태 
    ...
    def fit(self, data):
        ...
        #Curriuculum 전략에 따라서 mixup_beta값과 mix_up domain값을 갱신
        self.current_epoch+=1
        #mixup_beta값 갱신
        ...
        # Init dataloaders
        #중요 : 현재 batch에 존재하는 domain들 종류의 수가 2개 이상이라면 
        if self.dpb>1:
            # DistributedBalancedSampler: batch에 각 domain마다 같은 수의 이미지들이 존재하도록 보장 
        ...
        # Init logger values
        #AGG loss(Original Classification loss)
        sem_loss = 0.
        mimg_loss = 0.
        mfeat_loss = 0.
        ...
        for i, (inputs, _, domains, labels) in enumerate(dataloader):
            ...
            #scalar였던 label을 one-hot vector로 바꿈
            ...
            ##3. input level mixup에 대해서 forward
            ...
            if self.end_to_end:
                ...
                #image-level mixup loss 계산 
                mixup_img_loss = self.mixup_criterion(mixup_img_predictions, mixup_labels)
                #전체 loss : original classification loss+ image-level mixup loss + feature-level mixup loss
